...
outPath = './data/bValDF_less-than-50.pkl'
figPath = './figs'
cumulative = True
...
for nr in range(3):
  for nc in range(2):
    ...
    data = np.column_stack((df[tags[i]+'Cu'], df[tags[i]+val+'Cu'], df[tags[i]+val+'ErrCu']))
    # print(data)
    data = data[(data[:,0]>0), :]
    x = data[:, 0]
    y = data[:, 1]
    ...
    ax[nr,nc].errorbar(x[ind], y[ind], dy[ind], c='k', alpha=0.3, label=r'observed: $r_{corr}$=%.2f ($p_{corr}$=%.1e)' % (rcorr, pcorr))
    if not cumulative: 
      ax[nr,nc].plot(x[ind], linfunc(x[ind], c, d), c='green', lw=2, label=r'b=%.2f+%.3f*%s-->$R^2$=%.1f%%' % (c, d, metric[i], R2))
...
if cumulative:
  figname = './figs/fits_cumulativevalues-less-than-50.pdf'
else:
  figname = './figs/far-value/b-valVsSR_non_cum.pdf'
